...
def find_site(show, indice):
  ...
  req_search = requests.get(url) # makes request to URL and downloads/stores everything 
  req_search.raise_for_status() # checks to see if request was successful (code 200)
  soup_search = bs4.BeautifulSoup(req_search.text, "html.parser") # parses the whole site
  matches = soup_search.select(show_HTML[indice]) # searches parsed site for "blocks" 
...
def find_playbill(show, show_url, indice):
  ...
  if (show == 0): # i.e., if no show was found on any site
    ...
    for chunk in req_bill.iter_content(100000): # from ABSWP: write in chunks, useful for larger things
  ...
  # NDA na escolha da lista de matches, resultando em indice += 1
